...
def get(Dataobj, args={}, vargs={}):
    ...
    for i in need + ["SoundSpeed"]:
        if i not in Dataobj.quantities.keys() and i not in vargs.keys():
            ...
            return Dataobj
    quantities = {
        "Temperature": Dataobj.quantities["SoundSpeed"] ** 2
        * Dataobj.quantities["mu"]
        * Dataobj.quantities["mp"]
        / Dataobj.quantities["gamma"]
        / Dataobj.quantities["k"]
    }
    ...
    try:
        if AvgTh_cal:
            ...
            quantities = {
                "AvgTh_"
                + meanstr
                + "Temperature": Mean.Mean(Dataobj, "prs", vargs)
                / Mean.Mean(Dataobj, "rho", vargs)
                * Dataobj.quantities["mu"]
                * Dataobj.quantities["mp"]
                / Dataobj.quantities["k"]
            }  # update here
