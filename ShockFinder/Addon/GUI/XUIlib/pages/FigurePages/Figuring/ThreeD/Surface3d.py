...
def page(self):
    ...
    def load_saved(event):
        ...
        try:
            self.tkobj.io_recv(
                "Loading 3D Surface configuration",
                savedbox.get(),
                "...",
                color="blue",
            )
            savedcfg = self.hdf5handler.read_config(
                "3D", "Surfaces", savedbox.get()
            )
    ...
    def del_saved(event):
        if savedbox.get() == "" or not messagebox.askokcancel(
            "Delete Confirm",
            "Delete 3D Surface configuration " + savedbox.get() + " ?",
        ):
            return
        self.tkobj.io_recv(
            "Deleting 3D Surface configuration", savedbox.get(), "...", color="blue"
        )
    ...
    savedbox = ttk.Combobox(
        ...
        width=17,
        height=10,
        values=() + self.hdf5handler.read_config("3D", "Surfaces"),
    )
    ...
    box = self.add_row(bigbox)
    self.P3DSur_Qt = ttk.Combobox(
        box, width=17, height=10, values=self.avqt[2] + self.avqt[3]
    )
    ...
    box = self.add_row(bigbox)
    self.P3DSur_index = ttk.Combobox(
    ...
    Label(self.add_row(bigbox, bx=120), text="=" * 500).place(
        x=0, y=0, anchor="nw"
    )  # begin
    ...
    def drawsur3d(event):
        ...
        try:
            ...
            if self.P3DSur_Qt.get() not in self.pageargs["Infobj"].database.data[ind].quantities:
                raise IndexError(f'The simulation dose not run to {ind}')
            
            v = (
                self.pageargs["Infobj"]
                .database.data[ind]
                .quantities[self.P3DSur_Qt.get()]
                * factor
            )
            if v.ndim == 2:  # 2d:
                if (
            ...
            self.pageargs["Infobj"].Config["Painter"]["P3D"].surface(
                surface, **figureinfo
            )
            self.tkobj.io_recv("Operation completed", color="green")
        except Exception as err:
        ...
        try:
            if entry_save != "":
                if entry_save.get() in self.hdf5handler.read_config(
                    ...
                    "Rewrite Confirm",
                    "Rewrite 3D Surface configuration " + entry_save.get() + " ?",
                ):
                    return
                self.tkobj.io_recv(
                    "Saving 3D Surface configuration",
                    entry_save.get(),
                    "...",
                    color="blue",
                )
